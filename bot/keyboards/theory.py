...
def get_subject_keyboard(grade):
    ...
    markup.add(InlineKeyboardButton("Назад", callback_data="back_to_grade"))
    if (grade >= 7):
        markup.add(
...
def get_topic_keyboard(topics):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("Назад", callback_data="back_to_subject"))
    for topic in topics:
...
def get_theory_nav_keyboard(topic_id: int, is_read: bool):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("Назад", callback_data="back_to_topics"))
    if is_read:
